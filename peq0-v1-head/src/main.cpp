...
extern "C" void app_main(void)
{
    ...
    // --- IMU ---
    if (lsm6_init(PIN_SDA, PIN_SCL) != ESP_OK) { ESP_LOGE(TAG, "I2C bus init failed"); return; }
    uint8_t imu_addr, whoami;
    if (lsm6_read_who_am_i(&imu_addr, &whoami) != ESP_OK) { ESP_LOGE(TAG, "IMU not found"); return; }
    ESP_LOGI(TAG, "IMU at 0x%02X, WHO_AM_I=0x%02X", imu_addr, whoami);
    if (lsm6_configure_default() != ESP_OK) { ESP_LOGE(TAG, "IMU configuration failed"); return; }

    // --- Wi-Fi / UDP (auto-connects if previously provisioned) ---
    if (wifi_udp_init() != ESP_OK) { ESP_LOGE(TAG, "Wi-Fi/UDP init failed"); return; }

    // --- BLE provisioning ---
    ble_provision_cfg_t bcfg = {
        .on_provision      = on_provision,
        .on_wearable_id    = on_wearable_id,
        .on_expected_pi_id = on_expected_pi_id,
        .status_getter     = status_getter,
    };
    if (ble_provision_init(&bcfg) != ESP_OK) {
        ESP_LOGE(TAG, "BLE init failed");   // not fatal
    }
